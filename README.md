...
6. В **«Внешнем виде»** включите **«Переводы над фразами»**: готовый словарь показывается прямо в субтитрах ячейками, русский перевод — мелко сверху, английский или пиньинь — основным текстом. Длинные переводы переносятся внутри ячейки; иероглифы остаются строкой снизу. Клик по ячейке открывает полный перевод. Режим сохраняется для страницы и не делает дополнительных AI-запросов. Пока перевод готовится или словарь пуст, остаётся обычная строка; для фраз, отсутствующих в словаре, перевод не придумывается.
...
Обычный режим использует SenseVoiceSmall с явно заданным языком и нормализацией текста. Аудио декодируется последовательно блоками не более минуты; разрез по возможности проходит через тихую паузу. Исходные отсчёты не пропускаются. Это ограничивает рабочую память на длинных записях, но не делает распознавание безошибочным.
...
**Дополнительная модель Nano:** `SUBSANYWHERE_ASR_MODEL=nano` включает Fun-ASR-Nano-2512; обычный режим — `sensevoice`. Нужен полный локальный каталог модели, включая `Qwen3-0.6B`; произвольный путь задаётся `SUBSANYWHERE_NANO_MODEL_DIR`. В нативном режиме стандартный путь — `~/.cache/modelscope/hub/models/FunAudioLLM/Fun-ASR-Nano-2512`. В Docker модель нужно отдельно смонтировать read-only и явно передать переменные окружения; основной импорт по-прежнему переносит только SenseVoice/VAD. Распознавание не скачивает модели автоматически. Nano запускается на CPU в float32, без случайного выбора слов и без автоматического перехода на другую модель при нехватке памяти.

На коротком китайском образце Nano **не показала выигрыша** и потребовала заметно больше памяти, поэтому она не включена по умолчанию. Её пословное выравнивание недоступно в проверенном checkpoint; таймкоды остаются основанными на VAD. Проверены короткие нативные прогоны SenseVoice на английском/китайском и Nano на китайском; это не оценка процента ошибок на ваших видео и не проверка Nano в Docker.
