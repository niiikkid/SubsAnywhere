...
- Личный словарь неизвестных слов: сохранение из ячейки субтитров, мягкая зелёная подсветка, блекло-бирюзовая для выученных и локальная веб-панель с иероглифами, пиньинем и переводом.
...
2. Откройте **«Неизвестные слова ↗»** в окне расширения или [веб-панель](http://127.0.0.1:43817/words). В ней — иероглифы, пиньинь и русский перевод; для английского — исходное слово и перевод. Кнопка **«Повторять»** открывает крупный режим обучения: слова из списка «На повторение» показываются по одному, с номером текущего слова, переводом и кнопкой «Следующее слово». В конце можно начать круг заново или выйти. Отметка «Выучено» переносит слово в отдельный список, не удаляя его; его всегда можно вернуть на повторение. Подсветка обновляется при возвращении к видео и периодически во время просмотра.
3. Словарь хранится в SQLite `/data/words.sqlite3`, в постоянном Docker-томе `subsanywhere_subtitles`. Перезапуск, пересборка и обычный `docker compose down` не удаляют его. **Не используйте `down -v`**, если хотите сохранить данные. Для резервного копирования остановите сервер и скопируйте весь том `/data`, включая базу словаря.
