...
- Позволяет менять положение, размер и цвет субтитров, а также цвет и непрозрачность фона; для SRT — вручную настроить смещение и скорость.
...
Для YouTube сервер берёт актуальные cookies из Chrome при каждом запросе. В Chrome нужно быть авторизованным в YouTube; cookies никуда не сохраняются и не передаются за пределы компьютера.
...
5. Во время локального распознавания окно показывает процент, число обработанных речевых сегментов и примерное оставшееся время. Оценка появляется после первых процентов обработки и уточняется по фактической скорости.
6. Окно расширения можно закрыть: работа продолжится на локальном сервере. При следующем открытии прогресс восстановится, а готовая дорожка подключится автоматически.

При создании своих субтитров детектор речи настроен на паузы около 0,4 секунды и ограничение непрерывного сегмента около 8 секунд. Сегменты не склеиваются обратно; таймкоды берутся из аудио, а не вычисляются по длине текста. Это уменьшает длинные блоки, но не обеспечивает пословную точность. Для применения к уже созданной дорожке повторно нажмите «Создать свои субтитры»; готовые дорожки YouTube не меняются.
