...
**0.10.0 — кандидат первого выпуска, не заявление о готовности 1.0.** Проверки и незакрытые сценарии перечислены в [матрице приёмки](docs/RELEASE.md#матрица-приёмки). [Обслуживание и диагностика](docs/RELEASE.md) · [Данные и приватность](docs/PRIVACY.md).

## Возможности

- Одна выбранная оригинальная дорожка из HTML5-плеера, включая доступные фреймы, либо свой SRT: UTF-8 / Windows-1251, до 5 МБ.
- Полный перевод английской реплики и список переведённых фраз через собственный ключ DeepSeek или OpenAI.
...
Эта проверка может обратиться к YouTube, но **не запускает распознавание**. Для создания своих субтитров выберите язык речи: английский или китайский; при «Авто» создание по-прежнему использует китайский. Docker по умолчанию работает без cookies; вход в YouTube в обычном Chrome не авторизует контейнер. Если YouTube требует вход, см. [необязательное подключение cookies](docs/RELEASE.md#необязательные-cookies-youtube).
