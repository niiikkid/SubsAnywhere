...
Оригинальные субтитры поверх видео, перевод слов и выражений на русский, китайские субтитры с пиньинем для YouTube.
...
- Перевод английских слов и выражений: обычное значение и значение в текущей реплике через собственный ключ DeepSeek.
- На YouTube — получение готовой китайской дорожки `zh`, `zh-Hans` или `zh-Hant`, включая автоматическую, если она доступна. Пиньинь выводится над иероглифами; перевод китайской реплики содержит краткий словарь `пиньинь — русский`.
...
5. Для перевода сохраните собственный ключ и выберите модель в **«Настройках» → «Перевод DeepSeek»**. Нажатие на слово/выражение открывает перевод; повторное нажатие или крестик закрывает подсказку.
...
При открытии окна на YouTube расширение проверяет сервер и сохранённое задание, затем получает доступную готовую китайскую дорожку. Эта проверка может обратиться к YouTube, но **не запускает распознавание**. Docker по умолчанию работает без cookies; вход в YouTube в обычном Chrome не авторизует контейнер. Если YouTube требует вход, см. [необязательное подключение cookies](docs/RELEASE.md#необязательные-cookies-youtube).
