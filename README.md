...
3. Откройте **«Слова и предложения ↗»** в окне расширения или [веб-панель](http://127.0.0.1:43817/words). Слова и предложения находятся в отдельных разделах. Для китайских слов и предложений кнопка **«Разобрать»** запускает единый разбор: сначала отдельным запросом заново получает пиньинь по исходным иероглифам, затем — русский перевод, значения слов и служебных частей, простое объяснение грамматики и короткий пример. В повторении показываются пиньинь, перевод и компактный разбор. **«Обновить разбор»** заново выполняет оба запроса; открытие карточки само по себе не обращается к ИИ. Новый результат сохраняется целиком только после успешного завершения; при ошибке предыдущий остаётся. В панели **«Модель ИИ»** можно отдельно выбрать OpenAI или DeepSeek, загрузить доступные модели и сохранить выбор только для обучения. Ключи используются уже сохранённые в расширении; выбор модели субтитров не меняется. Запросы разбора могут быть платными. У каждого раздела свои списки «На повторение»/«Выученные» и свой режим повторения. **«Скачать TSV»** выгружает все сохранённые слова: слово, пиньинь для китайского и русский перевод.
